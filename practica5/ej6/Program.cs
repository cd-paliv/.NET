...
namespace ej8
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($"\nAcceso al elemento B[1, 2] = {B[1, 2]}");
...
/*
        ...
        public void Imprimir(string formato)
        {
            for (int i = 0; i < _filas; i++){
                for (int j = 0; j < _columnas; j++){
                    Console.Write(m[i, j].ToString(formato) + " ");
                }
                Console.WriteLine();
            }
        }
        ...
        public double[] DiagonalPrincipal //esq sup izq a esq inf der
        {
            get{
                if (_filas != _columnas){
        ...
        public double[] DiagonalSecundaria //esquina sup der a esquina inf izq
        {
            get{
                if (_filas != _columnas){
                    throw new ArgumentException ("ERROR: La matriz no es cuadrada");
                ...
                for(int i = 0; i < _filas; i++){
                    for(int j = 0; j < _columnas; j++){
                        ...
                        for(int z = 0; z < matriz.getColumnas(); z++){
                            mul[i, j] += m[i, z] * matriz[z, j]; //hago ({a11·b11 + a12·b21 + a13·b31 + a14·b41} = {0·0 + 1·4 + 2·8 + 3·12} para el primer elemento de C)
                        }
                    }
                }
